        ...
        case WM_INPUT: {
            ...
            switch (raw->header.dwType) {
                case RIM_TYPEKEYBOARD: {
                    ...
                    if (binds[p1Jump].contains(vkey))
                        inputType = PlayerButton::Jump;
                    else if (binds[p1Left].contains(vkey))
                        inputType = PlayerButton::Left;
                    else if (binds[p1Right].contains(vkey))
                        inputType = PlayerButton::Right;
                    else {
                        player = Player2;
                        if (binds[p2Jump].contains(vkey))
                            inputType = PlayerButton::Jump;
                        else if (binds[p2Left].contains(vkey))
                            inputType = PlayerButton::Left;
                        else if (binds[p2Right].contains(vkey))
                            inputType = PlayerButton::Right;
                        else
                            shouldEmplace = false;
                    }
